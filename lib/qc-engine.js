...
export function calculateVerdict(checklist, results, placements = [], mediaType = 'text') {
  ...
  if (totalChecked === 0) {
    verdict = {
      label: 'НЕ ПРОВЕРЕНО',
      color: '#a4b0be',
      bgGradient: 'linear-gradient(135deg, rgba(164, 176, 190, 0.15), rgba(164, 176, 190, 0.05))',
      icon: '⏳',
    };
 ...
 */
export function mergeAiResults(currentResults, aiResults, checklist = null) {
  const merged = { ...currentResults };
  if (!aiResults || typeof aiResults !== 'object') return merged;

  // Build ID lookup map from checklist
  const validMap = new Map();
  if (checklist?.sections) {
    for (const section of checklist.sections) {
      for (const item of section.items) {
        validMap.set(item.id.toLowerCase(), item.id);
        validMap.set(item.id.replace(/-/g, '_').toLowerCase(), item.id);
      }
    }
  }

  for (const [key, aiResult] of Object.entries(aiResults)) {
    if (key.startsWith('_')) {
      merged[key] = aiResult;
      continue;
    }
    ...
    const matchedId = validMap.get(cleanKey) || key;
